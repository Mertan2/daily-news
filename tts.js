(function () {
  ...
  const css = `
    .tts-bar {
      ...
      border: 1px solid var(--line, var(--border, #2a3441));
      border-radius: 12px;
      padding: 8px 12px;
      margin: 0 0 18px;
    ...
    .tts-bar button, .tts-bar select, .tts-bar input[type="range"] {
      background: var(--panel-2, var(--panel, #232b36));
    ...
    .tts-bar button:hover { border-color: var(--accent, #58a6ff); }
    .tts-bar button:disabled { opacity: 0.4; cursor: not-allowed; }
    .tts-bar label { color: var(--muted, #8b949e); font-size: 12px; display: inline-flex; align-items: center; gap: 6px; }
  ...
  function readables() {
    ...
    return Array.from(root.querySelectorAll('h1, h2, h3, p, li, td, .story, .day'))
      .filter(el => {
